...
| 단계 | 시기 | 내용 | 진행 |
|---|---|---|---|
| 1 | 0~3개월 | RAG 기초 (LlamaIndex/LangChain) + 도메인 기초 (LIPID MAPS, mzTab-M) | 진행 중 |
...
step3_query.py : 질의응답(query engine) 연결, LLM은 HuggingFaceInferenceAPI(Qwen2.5-7B-Instruct)사용
그라운딩 실패 사례 확인: 문서에 없는 질문("프랑스 수도는?")에도 모른다고 답하지 않고, 일반 지식으로 답변 -> Hallucination 위험 직접 확인

다음                    : 프롬프트 수정으로 "문서에 없으면 모른다고 답하기" 장치 실험 -> LLM 백엔드 선정 필요
